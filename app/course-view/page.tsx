...
const Main: React.FC<{ courseData: courseDataType; courseId: number }> = ({
  courseData,
  courseId,
}) => {
  ...
  useEffect(() => {
    if (courseData.currentChapter > 1 && !courseData.reviewed)
      setShowEnterReview(true);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  const data = {
  ...
  return (
    <>
      <HomeBt />
      <Name name={courseData.courseName} courseId={courseData.courseId} />
      {courseData.enrolled && (
        <>
          <div className="mt-2">
          ...
          <div className="flex items-center mb-6 mx-4">
            ...
            <div className="w-full h-2 rounded-full bg-gray-100">
              <div
                ...
                style={{
                  width: `${
                    courseData.completed ? 100 : courseData.progress.toFixed(2)
                  }%`,
                }}
              />
            </div>
      ...
      {courseData.enrolled ? (
        courseData.completed ? (
          <p className="fixed bottom-0 left-0 w-full bg-blue-500 text-white py-2 text-center">
            Completed
          </p>
        ) : (
          <ContinueLearningBT
